...
def add_cart(request,product_id):
    ...
    if request.method=='POST':
        ...
        for item in request.POST:
            ...
            try:
                variation=Variation.objects.get(product=product,variation_category__iexact=key,variatio_value__iexact=value)
    ...
    if is_cart_item_exits:
        ...
        if product_variation in ex_var_list:
             ...
             iten_id=id[index]
             item=CartItem.objects.get(product=product,id=iten_id)
             item.quantity+=1
             item.save()
